...
function buildReport(r) {
  ...
  cta.innerHTML = '<div><h3>Ready to go deeper?</h3><p>Work with a qualified Mind Flow coach and accelerate everything WIZ has started today.</p></div><div class="cta-btns"><button class="btn-primary" onclick="window.open(\'https://mindflowpro.com\',\'_blank\')">Book a Session</button><button class="btn-secondary" onclick="window.print()">Save Report</button></div>';
  rv.appendChild(cta);
...
async function startSession() {
  ...
  try {
    var opening = await callWIZ(null);
    setLoading(false);
